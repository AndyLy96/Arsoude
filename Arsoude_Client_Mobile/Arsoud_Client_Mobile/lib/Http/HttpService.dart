...
Future<List<Event>> getEvents() async{
  try{
    ...
    var response = await dio.get('/Trail/GetEventList',  options: Options(
        contentType: "application/json"
    ));
    print(response);
    var listJson = response.data as List;
    var listTrail = listJson.map((elementJson){
      return Event.fromJson((elementJson));
    }).toList();
    return listTrail;
  }
  catch (e){
...
Future<String> changeNotificationSettings(NotificationSettings notificationSettings) async{
  try{
    configureDio();
    var response = await dio.post('/User/SendNotificationSettings', data: notificationSettings.toJson(), options: await getOptions());
...
Future<List<Randonne>> getSearchTrails(String searchWord) async{
  try{
    configureDio();
    FilterDTO filterDTO = FilterDTO();
    filterDTO.keyWord = searchWord;
    var response = await dio.post('/Trail/GetFilteredTrails', data: filterDTO.toJson(), options: await getOptions());
    //print(response);
    var listJson = response.data as List;
    var listTrail = listJson.map((elementJson){
      return Randonne.fromJson((elementJson));
