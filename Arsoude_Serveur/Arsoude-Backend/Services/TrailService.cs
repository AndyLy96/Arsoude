...
namespace Arsoude_Backend.Services
{
    public class TrailService : ControllerBase
    {
        ...
        public async Task<Hike> CreateHike(Hike hike, IdentityUser user)
        {
            ...
            Trail trail = _context.Trails.Where(h => h.Id == hike.TrailId).FirstOrDefault();

            trail.TotalCompletions++;
        ...
        public async Task<List<Trail>> GetMostCompleted()
        {

            List<Hike> result = new List<Hike>();

            result = await _context.Hikes.Where(x => x.IsCompleted == true).ToListAsync();

            var group = result.GroupBy(x => x.TrailId)
                        .OrderByDescending(group => group.Count())
                        .Take(3)
                        .Select(group => group.Key);
            List<Trail> popularTrail = new List<Trail>();
            foreach(var id in group)
            {
                Trail trail = await _context.Trails.Where(x => x.Id == id).FirstOrDefaultAsync();
                popularTrail.Add(trail);
            }

            if(popularTrail.Count == 0)
            {
                popularTrail = await _context.Trails.Take(3).ToListAsync();
            }
        ...
        public async Task<List<Trail>> GetFilteredTrails(FilterDTO dto)
        {
            ...
            {
                query = query.Where(x => x.isPublic == true && x.IsApproved == true);
            }
        ...
        public async Task RateTrail(int trailId, string rating, IdentityUser userIdentity)
        {
            ...
            if (trail != null)
            {
                if(currentUser != null)
                {
                    ...
                    List<TrailUser> nbVotes = await _context.TrailRatingUser.Where(x => x.TrailId == trailId).ToListAsync();
                    double votesTotalValue = 0;
                    foreach(var vote in  nbVotes)
                    {
                        votesTotalValue += vote.VoteValue;
                    }
                    double voteAverage = Math.Round((votesTotalValue / (double)nbVotes.Count), 2);

                    trail.Rating = voteAverage;
        ...
        public async Task<List<Event>> GetEvents()
        {
            ...
            result = await _context.Events.OrderByDescending(x => x.Id).ToListAsync();
