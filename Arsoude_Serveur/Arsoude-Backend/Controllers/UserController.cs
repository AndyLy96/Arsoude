...
namespace Arsoude_Backend.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> SendNotificationSettings(NotificationSettings notificationSettings)
        {
            IdentityUser user = await UserManager.FindByIdAsync(User.FindFirstValue(ClaimTypes.NameIdentifier));
            try
            {
                await _userService.ChangeSettings(notificationSettings, user);
                return Ok();
            }
            ...
            catch (NotificationSettingsNotFoundException)
            {
                return NotFound(new { Message = "These settings were not found" });
            }
        }
