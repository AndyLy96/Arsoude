...
namespace Arsoude_Backend.Controllers
{
    ...
    public class TrailController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<List<Trail>>> GetPopularTrails()
        {
            try{
                var listOfTrails = await _trailService.GetMostCompleted();
                return listOfTrails.ToList();
            }
            catch {
                return NotFound(new { Message = "No trail found" });
            }
        ...
        public async Task<ActionResult<List<Event>>> GetEventList()
        {
            try
            {
                var listOfEvents= await _trailService.GetEvents();
        ...
        [HttpGet]
        public async Task<ActionResult<List<UnityTrailDto>>> GetTrailsDto()
        {
            var trails = await _context.Trails.ToListAsync();
            List<UnityTrailDto> trailDtos = new List<UnityTrailDto>();
            foreach (var trail in trails)
            {
                UnityTrailDto trailDto = new UnityTrailDto();
                trailDto.Id = trail.Id;
                trailDto.Description = trail.Description;
                trailDto.Location = trail.Location;
                trailDto.Name = trail.Name;
                trailDto.Type = trail.Type;
                trailDto.ImageUrlList = new List<string>();
                try
                {
                    for (int i = 0; i < trail.ImageList.Count(); i++)
                    {
                        var img = trail.ImageList[i];
                        trailDto.ImageUrlList.Add(img.ImageUrl);
                    }
                    trailDto.ImageUrlList.Add(trail.ImageUrl);
                }
                catch (Exception ex)
                {
